...
[Page Meta](#meta) 和 [Page Data](#data) 都可以在客户端代码中使用。然而， Page Meta 是附加在页面路由记录上的，因此当用户进入你的站点时，所有页面的 Page Meta 都会立即被加载。相比之下， Page Data 是存储在单独的文件中的，只有在用户进入对应页面时才会被加载。
